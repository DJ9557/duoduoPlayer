...
#import "UNPhotoCell.h"
#import "UNShowImageViewController.h"

@interface UNPickPhotoViewController () <ZLPhotoCellDelegate>
...
@implementation UNPickPhotoViewController
...
- (void)loadSelectPhoto {
    ...
    NSMutableArray *selectAssest = [NSMutableArray array];
    ...
    for (NSNumber *n in self.selectIndexs) {
        ...
        [[ZLPhotoHelper sharedZLPhotoHelper] getFullScreenImageWithAsset:asset completion:^(UIImage *photo, NSDictionary *info) {
            ...
            [selectAssest addObject:asset];
        }];
    }
    
    if (self.completeHandle) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.completeHandle(selectPhoto,selectAssest);
        });
    }
}
- (void)rightItemClick {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    UNPhotoCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:reuseIdentifier forIndexPath:indexPath];
...
- (void)photoCell:(UNPhotoCell *)cell longPressImage:(UIImage *)image {
    ...
    [[ZLPhotoHelper sharedZLPhotoHelper]getOriginImageWithAsset:asset completion:^(UIImage *photo, NSDictionary *info) {
        UNShowImageViewController *vc = [[UNShowImageViewController alloc] init];
        vc.image = photo;
